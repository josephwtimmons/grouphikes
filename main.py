...
def list_events(
    ...
    max_miles: float | None = Query(default=None),
    start_date: str | None = Query(default=None),   # YYYY-MM-DD
    dog_friendly: str | None = Query(default=None), # "", "Yes", "No"
):
    date_filter = None
    if start_date:
    ...
    with Session(engine) as session:
        ...
        if mountain_id:
            stmt = stmt.where(Event.mountain_id == mountain_id)

        if pace:
            stmt = stmt.where(Event.pace == pace)

        if max_miles is not None:
            stmt = stmt.where(Event.distance_miles <= max_miles)
    ...
    # Normalize rows for Jinja: {% for ev, mtn in rows %}
    ...
    return templates.TemplateResponse(
        "events.html",
        {
            ...
            "filters": {
                "mountain_id": mountain_id,
                "pace": pace or "",
                "max_miles": "" if max_miles is None else max_miles,
